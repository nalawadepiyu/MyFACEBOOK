...
public class TwitterServlet extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {						
		...
		if (req.getParameterMap().containsKey("Author")) {
			sendName=req.getParameter("Author");
			req.setAttribute("sendName", sendName);
		}
		
		if (req.getParameterMap().containsKey("txtpostTwt")) {			
			String uuid = UUID.randomUUID().toString().replaceAll("-", "");		
			Entity tweet = new Entity("Tweets", uuid);
			tweet.setProperty("message", (String) req.getParameter("txtpostTwt"));
		...
		if (!sendName.equals("")){
			Filter propertyFilter =  new Query.FilterPredicate("author", FilterOperator.EQUAL, sendName);
